...
class Wallet:
    def __init__(self, network_reference):
        self.block_chain = Blockchain()
        self.network = network_reference
    ...
    def mine(self):
        ...
        self.network.broadcast(mining_broadcast_message)
    ...
    def pay(self, receiver, value):
        ...
        self.network.broadcast(transaction_broadcast_message)
    ...
    def verify_new_block(self, mining_broadcast_message):
        ...
        miner_addr = parsed[0]
        nonce = int(parsed[1])
        if miner_addr == self.user_addr:
            # Validate his own block
            print("a node has verified new block")
            return
        if self.block_chain.verify_new_block(miner_addr, nonce):
            print("a node has verified new block")
            return
    ...
    def verify_transaction(self,transaction_broadcast_message):
        ...
        if self.block_chain.verify_transaction_signature(txn_message, sender_public_key_string, signature_string):
            print("a node has verified new transaction")
            return
